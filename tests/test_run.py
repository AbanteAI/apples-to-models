...
async def test_judge_move_with_exact_cards():
    ...
    mock_completion.choices = [
        AsyncMock(
            message=AsyncMock(content="Queen Elizabeth is the most graceful choice")
        )
    ]
    mock_completion.id = "test-id-4"
    ...
    async def mock_get_stats_case4(*args, **kwargs):
        return {
            "data": {
                "id": "test-id-4",
                "tokens_prompt": 10,
    ...
    with patch("benchmark.model_utils.os.getenv", return_value="test-key"), patch(
        "benchmark.model_utils.AsyncOpenAI", return_value=mock_client
    ), patch("benchmark.model_utils.get_generation_stats", new=mock_get_stats_case4):
        card, thinking, log_path = await model_judge_move(game, "test-model")
