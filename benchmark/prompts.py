...
def format_judge_decision(
    decision: JudgeDecision, green_card: str, played_cards: List[str]
) -> str:
...
def format_player_move(
    move: PlayerMove, green_card: str, for_player: bool = False
) -> str:
    ...
    if for_player:
        return (
            f"For the green card '{green_card}', {move.thinking} | {move.played_card}"
        )
    return f"For the green card '{green_card}', I play: {move.played_card}"


def create_game_history_messages(
    game: Game, current_round: Round, player_idx: Optional[int] = None
) -> Messages:
        ...
        for move_player_idx, move in past_round.moves.items():
            if move_player_idx == player_idx:
                messages.add_assistant(
                    format_player_move(move, past_round.green_card, True)
                )
            else:
                messages.add_user(
...
def create_player_messages(game: Game, player_idx: int) -> Messages:
    ...
    for move_player_idx, move in current_round.moves.items():
        if move_player_idx == player_idx:
            messages.add_assistant(
                format_player_move(move, current_round.green_card, True)
            )
        else:
            messages.add_user(
                f"Player {move_player_idx + 1}: "
...
def create_judge_messages(game: Game) -> Messages:
    ...
    messages.add_user(f"The green card is: {current_round.green_card}")
    messages.add_user(
        f"You are Player {current_round.judge + 1} and the judge for this round."
    )
