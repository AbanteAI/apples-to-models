...
def generate_cumulative_wins_chart(game: Game) -> str:
    ...
    for player_id, wins in player_wins.items():
        plt.plot(
            range(len(wins)),
            wins,
            label=f"{game.players[player_id].name} (Player {player_id + 1})",
            marker="o",
        )
...
def generate_win_percentage_chart(game: Game) -> str:
    ...
    for player_id, player in game.players.items():
        player_names.append(f"{player.name} (Player {player_id + 1})")
...
"""
    ...
    for idx, stats in standings:
        html += f"""
            <li>{stats['name']} (Player {idx + 1}): {stats['wins']} win{'s' if stats['wins'] != 1 else ''}</li>"""
...
def _generate_round_html(round: Round, players: Dict, player_stats: Dict) -> str:
    ...
    <div class="round">
        <div class="round-header">
            ...
            <p><strong>Judge:</strong> {judge_name} (Player {round.judge + 1})</p>
        </div>
"""
    ...
    html += f"""
        <div class="judge-section">
            <h4>👨‍⚖️ Judge: {judge_name} (Player {round.judge + 1})</h4>"""
    if round.decision:
        html += f"""
            <p><strong>Decision:</strong> {round.decision.reasoning}</p>
        </div>
        <div class="winner-section">
            <h4>🏆 Winner: {players[round.decision.winning_player].name} (Player {round.decision.winning_player + 1})</h4>
    ...
    for player_idx, player in non_judge_players:
        if player_idx in round.moves:
            ...
            <div class="{submission_class}">
                <p><strong>{player.name} (Player {player_idx + 1})'s Card:</strong> "{move.played_card}"</p>
                <div class="thinking">
            ...
            <div class="submission waiting">
                <p>Waiting for {player.name} (Player {player_idx + 1}) to play a card...</p>
            </div>"""
